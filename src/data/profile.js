 ...
 * 各科目は次の3項目を必ず含める:
 *   name        科目名
 *   learning    学んだこと    : 2〜3文。何を扱い、何が身についたかを具体的に書く
 *   application 関連する制作・活用例: その学びを制作事例やハッカソン等で
 *               実際にどう使ったか。1文程度
 * ---------------------------------------------------------------------- */
...
export const courses = [
  {
    name: 'アルゴリズムとデータ構造',
    learning:
      '計算量（時間・空間）を意識した設計の重要性を学んだ。配列・連結リスト・木構造・ハッシュ表それぞれの得意不得意を、実際に計算量を比較しながら理解した。',
    application:
      '学内サークル向け備品予約Webアプリで、予約の重複チェックにハッシュ集合を用いてO(1)の検索に落とし込む際に活かした。',
  },
  {
    name: 'データベース論',
    learning:
      '正規化の考え方とトランザクションのACID特性を学んだ。同時アクセス時にデータの整合性が崩れる具体例を演習で体験し、制約設計の重要性を理解した。',
    application:
      '備品予約Webアプリで、同一機材・同一時間帯の予約をDBのユニーク制約で拒否する設計に直接応用した。',
  },
  {
    name: 'オペレーティングシステム',
    learning:
      'プロセスとスレッドの違い、排他制御（ミューテックス・セマフォ）の仕組みを学んだ。デッドロックが発生する条件を演習コードで実際に再現し、回避策を検証した。',
    application:
      'Webアプリのバックエンドで複数リクエストが同時に来た際の排他制御を設計するときの土台になった。',
  },
  {
    name: 'ネットワーク工学',
    learning:
      'TCP/IPの階層モデルとHTTPのリクエスト/レスポンスの流れを学んだ。パケットキャプチャツールで実際の通信を観察し、教科書の図と実測値を突き合わせて理解を深めた。',
    application:
      'ハッカソンでAPIのレスポンスが遅い原因を切り分ける際、ネットワーク層とアプリケーション層のどちらに問題があるかを判断する基礎になった。',
  },
  {
    name: 'ソフトウェア工学',
    learning:
      '要件定義からテスト・保守までの開発プロセス全体と、コードレビューの観点を学んだ。グループ演習でレビューを受ける側・する側の両方を経験した。',
    application:
      '個人開発でもPull Requestベースの開発フローとCIによる自動テストを取り入れる習慣につながった。',
  },
  {
    name: '情報セキュリティ',
    learning:
      'SQLインジェクションやXSSなど代表的な脆弱性の仕組みと対策を学んだ。演習環境で実際に脆弱なコードを攻撃し、対策の前後で挙動が変わることを確認した。',
    application:
      '個人開発のWebアプリでは、入力値のバリデーションとORM経由のクエリ発行を徹底する意識につながった。',
  },
]
